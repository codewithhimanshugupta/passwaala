...
export interface DisputeModalHandle {
  /** Open the modal, optionally auto-raising a dispute with the given reason. */
  openWithReason: (reason?: string) => void;
}

export const DisputeModal = forwardRef<DisputeModalHandle, {
  orderId: string;
  ...
  // A reason queued by openWithReason() to auto-submit once the modal has loaded
  // the (absent) dispute — used by the "I didn't get my refund" flow.
  ...
  useImperativeHandle(ref, () => ({
    openWithReason: (r?: string) => {
      pendingReasonRef.current = r?.trim() || null;
      setOpen(true);
    },
  }), []);
  ...
  // Auto-submit a queued reason (from openWithReason) once we've confirmed there
  // is no existing dispute yet and we're inside the window.
  useEffect(() => {
    if (!open || dispute !== null || !withinWindow || busy) return;
    const queued = pendingReasonRef.current;
    if (!queued) return;
    pendingReasonRef.current = null;
    void raise(queued);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [open, dispute, withinWindow]);
