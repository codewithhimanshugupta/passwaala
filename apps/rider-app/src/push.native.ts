/**
 * Native (iOS/Android) Expo push-token registration for the rider app. Metro
 * loads this on iOS/Android while the web bundle keeps the no-op push.ts.
 * Registers the device's Expo push token with the backend so the server can send
 * remote new-job / alert pushes; unregisters on logout. Never throws.
 */
