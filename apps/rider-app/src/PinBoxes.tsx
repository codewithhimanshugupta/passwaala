 ...
 * with the full-width fields around it. Controlled: the parent holds the value
 * string (0–length digits). Masked by default.
 */
export function PinBoxes({
  ...
  function handleChange(idx: number, raw: string) {
    ...
    if (cleaned.length > 1) {
      const joinedPaste = cleaned.slice(0, length);
      onChange(joinedPaste);
      const target = Math.min(joinedPaste.length, length - 1);
      setTimeout(() => refs.current[target]?.focus(), 0);
      if (joinedPaste.length === length) onComplete?.(joinedPaste);
      return;
    }
...
const styles = StyleSheet.create({
  ...
  eyeText: { fontSize: theme.font.small, color: theme.color.accent, fontWeight: '600' },
