/**
 * Web-only looping alert sound for new orders. Uses the Web Audio API to
 ...
 * new-order beep is silent. Call this from the FIRST user gesture (e.g. the
 * login "Verify" tap / first touch on the app root): it creates + resumes the
 * context and plays one near-silent tick so the context is "running" and later
 * beeps actually sound. Safe/no-op off the web.
 */
