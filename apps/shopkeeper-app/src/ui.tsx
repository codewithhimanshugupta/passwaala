...
/**
 * Reusable UI primitives for the shopkeeper app: Screen, Card, Button, Badge,
...
function bannerPalette(tone: BannerTone): { bg: string; fg: string; border: string } {
  switch (tone) {
    ...
    case 'info':
    default:
      return { bg: theme.color.accentSoft, fg: theme.color.accentDark, border: '#CDD3F6' };
  }
}
...
export function OtpBoxes({
  ...
  function handleChange(idx: number, val: string) {
    // On web, onChangeText can fire with the current cell's existing value
    // prepended to the new key (e.g. "6" → typing "9" fires "69"). Strip the
    // old digit so we only use what was actually typed.
    const stripped = val.replace(/\D/g, '');
    const incoming = stripped.startsWith(digits[idx] ?? '') && stripped.length > 1
      ? stripped.slice(1)
      : stripped;
    const d = incoming.slice(0, length - idx);
    if (!d) return;
    const next = [...digits];
